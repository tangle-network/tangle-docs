...
- ***`protocol`***: Specifies the protocol (e.g., Tangle, Eigenlayer) to which the report will be submitted.
...
For more information on implementing custom event listeners, refer to the [Event Listeners documentation](./event-listeners).
...
Reports can be closely integrated with the [Jobs system](./jobs) to provide comprehensive monitoring and slashing capabilities. For example, you can create reports that are triggered by specific job outcomes or that monitor the overall performance of your AVS jobs.
