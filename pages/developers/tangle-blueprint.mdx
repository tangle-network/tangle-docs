...
The `#job` macro is used to define individual tasks or jobs within a blueprint. It allows you to specify various parameters and metadata for each job. #[job(id = <job_id>, params(<param_list>), result(<result_type>), verifier(evm = "<contract_name>"))]
...
The `#report` macro is used to define reporting functions that can be called after job execution to provide insights or summaries. #[report(
job_id = <job_id>,
params(<param_list>),
result(<result_type>),
