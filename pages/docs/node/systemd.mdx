...
Before following this guide, ensure that your machine's environment is set up and the Tangle binary is compiled. If you haven't done so, please refer to the [Requirements](../hardware/) page.
...
<Tabs items={['Full Node Setup', 'Validator Node Setup', 'Full Node with EVM Trace Setup']}>
  ...
  <Tabs.Tab>
    ...
    #### Generate and Store Keys
      ...
      at [Start Validating](./validator/requirements) to start validating.

  </Tabs.Tab>
  <Tabs.Tab>
  The following is the service configuration file, use this while completing the Full Node guide.
            **Note:** To run with evm trace, you should use a binary built with `txpool` flag, refer to [Binaries](../node-software/#binaries) page for more details.
