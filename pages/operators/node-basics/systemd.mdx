...
Before following this guide, ensure that your machine's environment is set up and the Tangle binary is compiled. If you haven't done so, please refer to the [Requirements](https://docs.tangle.tools/node/hardware/) page.
...
```
wget wget https://github.com/webb-tools/tangle/releases/download/<VERSION>/tangle-linux-amd64
```

For example, at the time of writing this document, the latest release is v0.6.1 and the link would be as follows

```
wget https://github.com/webb-tools/tangle/releases/download/v0.6.1/tangle-mainnet-linux-amd64
```
...
<Tabs items={['Full Node Setup', 'Validator Node Setup', 'Full Node with EVM Trace Setup']}>
  ...
  <Tabs.Tab>
    ...
    #### Generate and Store Keys
        ...
        We need to generate the required keys for our node. For more information on these keys, please see the [Required Keys](https://wiki.polkadot.network/learn-cryptography) section.
      ...
      at [Start Validating](https://docs.tangle.tools/node/validator/requirements/) to start validating.

  </Tabs.Tab>
  <Tabs.Tab>
  The following is the service configuration file, use this while completing the Full Node guide.
            **Note:** To run with evm trace, you should use a binary built with `txpool` flag, refer to [Binaries](https://docs.tangle.tools/node/node-software/#binaries) page for more details.
...
To setup monitoring for your node, please refer to the [monitoring](https://docs.tangle.tools/node/monitoring/quickstart/) page.

## Begin Validating

Now that your node is setup, [continue onto our Validator guides to understand token bonding and more.](https://docs.tangle.tools/node/validator/requirements/)
